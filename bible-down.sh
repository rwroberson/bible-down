...
abbarray=(Ge Exo Lev Num Deu Josh Judg Ruth 1Sm 2Sm 1Kgs 2Kgs 1Chr 2Chr Ezra Neh Est Job Psa Prov Eccl Song Isa Jer Lam Eze Dan Hos Joel Amos Oba Jonah Mic Nah Hab Zep Hag Zec Mal Matt Mark Luke John Acts Rom 1Cor 2Cor Gal Eph Phil Col 1Th 2Th 1Tim 2Tim Tit Philem Heb Jas 1Pt 2Pt 1Jn 2Jn 3Jn Jude Rev)
...
do
	...
	do
		...
		sed 's/###### /\n/g' | \
		sed 's/ /\t/' | \
		sed 's/AboutHelp.*$//;/By submitting your email.*$/d' | \
		grep -v '#' | \
		awk \
			-v b="${bookarray[$i]}" \
			-v a="${abbarray[$i]}" \
